    ...
    for episode in tqdm(
        ...
        if episode < args.warmup_episodes:
            ...
            while not cache_env.is_done():
                ...
                cache_masks = torch.tensor(
                    cache_env.masks, dtype=torch.float32
                ).reshape(-1)
                ...
                accumulated_cache_costs.append(reward / cache_env.scale)
                ...
                accumulated_cache_costs.append(reward)

            # Step 3: Overwrite the cache states in the environment before performing the small step
            env.large_step(cache_env.cache_status, cache_vehicle)
